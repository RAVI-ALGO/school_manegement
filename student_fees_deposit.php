...
if (isset($_REQUEST['schollar_no'])) {
    $schNO = $_REQUEST['schollar_no'];
    $selectQuery = mysqli_query($connection, "SELECT * FROM student_registraion WHERE schollar_no='$schNO'");
    ...
    $feesQuery = mysqli_query($connection, "SELECT * FROM class_fees WHERE class='$class'");
    $get_fees = mysqli_fetch_array($feesQuery);
    $full_fees = $get_fees['total_fees'];
    $feesQuery2 = mysqli_query($connection, "SELECT * FROM student_fees WHERE schollar_no='$schNO'");
    $sum = 0;
    foreach ($feesQuery2 as $list) {
        $sum += $list['last_transaction_amount'];
    }
    if (empty($sum)) {
        $remainFees = $full_fees;
    } 
    else {
        $remainFees=$full_fees-$sum;
    }
}
else
{
    $error="<div class='alert alert-warning alert-dismissible fade show' role='alert'>
    <strong>❎😭😭😭</strong>Invalid schollar no,Please go back.
...
if (isset($_POST['Paybtn'])) {
    ...
    echo$remainFees;
    $insertQuery = mysqli_query($connection, "INSERT INTO student_fees(schollar_no,name,father_name,class,section,mobile,total_fees,total_deposit,fees_remain,pay_date,last_transaction_amount)VALUES('$schNO','$stuname','$fathername','$class','$section','$mobileNo','$total_fees','$total_deposit','$remainFees','$paydate','$amount')");
    if ($insertQuery) {
...
<body>
    ...
    <h3 class="text-center bg-info">Student Fees Deposite</h3>
    <div class="container mt-4">
        <form method="POST">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-5 fs-3 text-center text-light bg-success"><span>Total Fees- <?php if(!empty($get_fees['total_fees'])){echo$get_fees['total_fees'];} ?></span>
                        <input type="text" name="totalfees" value="<?php echo $get_fees['total_fees']; ?>" hidden>
                    </div>
                    <div class="col-5 fs-3 text-center text-light bg-success"><span>Remaining Fees -<?php if(!empty($remainFees)){echo $remainFees;} ?></span>
                        <input type="text" name="Remainfess" value="<?php echo $remainFees; ?>" hidden>
                    </div>
                </div>
            </div>
            ...
            <div class="row justify-content-center mt-2">
                ...
                <div class="col-md-3"><input type="text" class="form-control text-capitalize" name="name" value="<?php echo $Fetch_data['first_name'] . "&nbsp;" . $Fetch_data['last_name']; ?>" readonly></div>
            </div>
            <div class="row justify-content-center mt-2">
                <div class="col-md-2">
                    <h4>Father's Name</h4>
            ...
            <div class="row justify-content-center mt-2">
                ...
                <div class="col-md-3"><input type="date" class="form-control" name="pay_date" required></div>
            </div>
            <div class="row justify-content-center mt-2">
                <div class="col-md-2">
                    <h4>Fees Amount</h4>
                </div>
                <div class="col-md-3"><input type="text" class="form-control" name="fess" value="" required></div>
            </div>
            <div class="row justify-content-center mt-4">
                <div class="col-md-2"></div>
                <div class="col-md-3"><input type="submit" class=" btn btn-primary" name="Paybtn" value="Pay Fees" ></div>
            </div>
    </div>
    </form>
    </div>
